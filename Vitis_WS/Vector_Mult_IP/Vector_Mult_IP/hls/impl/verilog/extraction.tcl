...
proc gen_data_dict_vivado {has_impl options_dict {reports_dict {}}} {
    get_report_option_vars $options_dict {report_level report_max_paths hls_reportdir vivado_reportdir vivado_reportbasename target_clk_period target_device hls_project hls_solution clock_name hls_impl_dir topmodule funcmodules bindmodules max_module_depth}
    if { $hls_impl_dir eq "" } {
    ...
    if { $topcell ne "" } {
      ...
      set celldict [dict create HAS_IMPL $has_impl ROOT_CELL [file dir $topcell] TOP_CELL $topcell DATA {} UTIL_DATA $util_data TOPMODULE $topmodule FUNCMODULES $funcmodules BINDMODULES $bindmodules]
    ...
    if { $report_level > 0 || $timing_met eq "FALSE" } {
      ...
      set tcell_props {PRIMITIVE_TYPE FILE_NAME LINE_NUMBER}
      ...
      foreach tpath $tpaths {
        ...
        foreach tcell [get_cells -quiet -of_objects $tpath] {
          set tcell_rel [file dirname $tcell]
          regsub "^$rootcell\[/\]" $tcell_rel "" tcell_rel

          set tcell_dict [dict create NAME $tcell]
          foreach prop $tcell_props {
            dict set tcell_dict $prop [get_file_name_prop $prop $tcell]
          }
...
proc get_all_vv_rpt_file_types {} {
  return [list status timing timing_paths utilization utilization_hierarchical design_analysis failfast power]
}

proc get_vv_rpt_file {filetype is_impl reportdir reportbasename {suffix ""}} {
...
proc generate_reports_vivado {is_impl options_dict} {
  ...
  run_report has_error $show report_power -file [get_vv_rpt_file "power" $is_impl $vivado_reportdir $vivado_reportbasename] -xpe [get_vv_rpt_file "power" $is_impl "." $vivado_reportbasename ".xpe"]
      ...
      if { ![dict exists $mod_dict RESOURCES] && ![dict exists $mod_dict LOCAL_RESOURCES] && ![dict exists $mod_dict CHILDREN] } {
        ...
        if { [dict exists $mod_dict CHILDREN] } {
          ...
          if { [llength $mod_children] } {
            set children_str [join [lsort $mod_children] " "]
            puts $out "$indent2<SubModules count=\"[llength $mod_children]\">${children_str}</SubModules>"
          }
        }
...
proc get_file_name_prop {prop_name obj} {
  ...
  if { [string length $val] && $prop_name eq "FILE_NAME" } {
...
proc get_cell_hier_data {cell celldict_var depth} {
  #puts "DBG: get_cell_hier_data: cell=$cell depth=$depth"
  ...
  set found_bindmodule false
  set orig_ref_name [dict_get_default $currdict ORIG_REF_NAME ""]
  if { [string length $orig_ref_name] } {
    foreach modulename [dict get $celldict BINDMODULES] {
      if { [string match "${modulename}*" $orig_ref_name] } {
        set found_bindmodule true
        break
      }
    }
  }
...
proc dict_get_default {adict args} {
    if { [llength $args] < 2 } {
        my_error "Expected at least 3 arguments to dict_get_default <dict> <dict-key1> \[dict-key2\] ...\[dict-keyN\] <default-value>"
    }

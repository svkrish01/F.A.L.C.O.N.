...
entity Canny_Filter_control_s_axi is
generic (
...
end entity Canny_Filter_control_s_axi;
...
architecture behave of Canny_Filter_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    constant ADDR_HIGH_THRESHOLD_DATA_0 : INTEGER := 16#20#;
    constant ADDR_HIGH_THRESHOLD_CTRL   : INTEGER := 16#24#;
    constant ADDR_LOW_THRESHOLD_DATA_0  : INTEGER := 16#28#;
    constant ADDR_LOW_THRESHOLD_CTRL    : INTEGER := 16#2c#;
...
-- ----------------------- Register logic ----------------
    ...
    High_Threshold       <= STD_LOGIC_VECTOR(int_High_Threshold);
    Low_Threshold        <= STD_LOGIC_VECTOR(int_Low_Threshold);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_High_Threshold(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_Low_Threshold(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then

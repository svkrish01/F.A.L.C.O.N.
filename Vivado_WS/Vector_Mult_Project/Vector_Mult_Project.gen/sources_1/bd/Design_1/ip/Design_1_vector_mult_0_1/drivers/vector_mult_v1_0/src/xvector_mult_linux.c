...
typedef struct {
    ...
    XVector_mult_uio_map maps[ MAX_UIO_MAPS ];
} XVector_mult_uio_info;

/***************** Variable Definitions **************************************/
static XVector_mult_uio_info uio_info;
...
static int uio_info_read_name(XVector_mult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVector_mult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVector_mult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVector_mult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVector_mult_Initialize(XVector_mult *InstancePtr, const char* InstanceName) {
	XVector_mult_uio_info *InfoPtr = &uio_info;
